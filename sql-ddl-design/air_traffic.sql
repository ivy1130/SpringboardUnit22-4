...
(
  ...
  flight INTEGER NOT NULL REFERENCES flights,
  seat TEXT NOT NULL
);
...
JOIN airlines a
ON f.airline = a.id
JOIN cities from_city
ON from_city.id = f.from_city
JOIN countries from_country 
ON from_country.id = f.from_country
JOIN cities to_city
ON to_city.id = f.to_city
JOIN countries to_country 
ON to_country.id = f.to_country;
